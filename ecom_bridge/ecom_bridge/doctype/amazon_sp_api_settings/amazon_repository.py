...
from ecom_bridge.ecom_bridge.doctype.amazon_sp_api_settings.amazon_sp_api_settings import (
	AmazonSPAPISettings,
)
	...
	def get_orders(self, created_after) -> list:
		...
		while True:
			...
			for order in orders_list:
				savepoint = f"amazon_order_{order.get('AmazonOrderId', 'unknown').replace('-', '_')}"
				try:
					...
					frappe.log_error(
						title=f"Amazon order sync failed: {order.get('AmazonOrderId')}",
